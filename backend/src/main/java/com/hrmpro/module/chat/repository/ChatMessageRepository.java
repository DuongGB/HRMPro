...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query("SELECT c FROM ChatMessage c WHERE (c.sender.id = :user1 AND c.receiver.id = :user2) " +
           "OR (c.sender.id = :user2 AND c.receiver.id = :user1) ORDER BY c.createdAt ASC")
